...
boolean Adafruit_MAX31856::begin(void) {
  ...
  // 4 samples on each measuremente (141ms on auto mode on 60Hz; 257ms on 1-shot mode 60Hz)
  setNumberOfSamples(MAX31856_SAMPLING_4_TIMES);
...
void Adafruit_MAX31856::setNumberOfSamples(max31856_sampling_t type) {
  uint8_t t = readRegister8(MAX31856_CR1_REG);
  t &= 0x0F;
  t |= (uint8_t)type & 0x70; // masking to 0111 on the first 4 bytes, keeping reserved as it is 
  writeRegister8(MAX31856_CR1_REG, t);
}

uint8_t Adafruit_MAX31856::getCR1(void) {
  uint8_t t = readRegister8(MAX31856_CR1_REG);
  return t;
}
